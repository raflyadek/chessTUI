...
func (gs *gameState) checkMove(pieceDestination string, board [8][8]string, whiteKingPositionCopy, blackKingPositionCopy string) error {
	...
	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
			...
			if player == "Black" {
				if board[i][j] != "" {
					if strings.Contains(pieces, board[i][j]) {
	...
	for i := -1; i < 2; i++ {
		for j := -1; j < 2; j++ {
			if player == "White" {
				...
				if board[toRowKing][toColKing] == "" || strings.Contains(strings.ToUpper(pieces), board[toRowKing][toColKing]) {
					possibleKingMove++
			...
			if player == "Black" {
				...
				if board[toRowKing][toColKing] == "" || strings.Contains(strings.ToLower(pieces), board[toRowKing][toColKing]) {
					possibleKingMove++
					possibleSquareKing = append(possibleSquareKing, toString)
				}
			}
	...
	if checkCounter == 1 {
					...
					if board[i][j] != "" {
						if strings.Contains(pieces, board[i][j]) {
							...
							if err == nil {
								...
								fmt.Printf("piece who can eat: %s, len: %d\n", board[i][j], len(pieceWhoCanEat))
								// pieceWhoCanEat++
								// return false
							}
						}
		...
		for i := 0; i < len(squareUntilCheck); i++ {
			for j := 0; j < 8; j++ {
				for k := 0; k < 8; k++ {
					if player == "White" {
						if strings.Contains(pieces, board[j][k]) && board[j][k] != "k" {
					...
					if player == "Black" {
						if strings.Contains(strings.ToUpper(pieces), board[j][k]) && board[j][k] != "K" {
							squareCheckRow := 8 - int(squareUntilCheck[i][1]-'0')
							squareCheckCol := int(squareUntilCheck[i][0] - 'a')
							fromByte1 := byte(97 + k)
							fromByte2 := byte(56 - j)
		...
		for j := 0; j < 8; j++ {
			...
			if board[i][j] != "" {
				if counterEmptyBoard != 0 {
					// fen += counterStr
					fen.WriteString(counterStr)
					counterEmptyBoard = 0
				}
				// fen += board[i][j]
				fen.WriteString((board[i][j]))
			} else {
				counterEmptyBoard++
